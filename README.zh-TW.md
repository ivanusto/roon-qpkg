...
| `ROON_MUSIC_PATH` | 自動偵測 Multimedia/Music/Public 共享 | 音樂庫 → 容器 `/Music` |
...
### 從狀態頁設定資料夾（1.2.0+，免 SSH）

狀態頁內建「資料夾設定」表單，可直接修改音樂庫／資料庫／備份三個掛載路徑，
並提示 NAS 上現有的共享資料夾。儲存後由主機端排程（每分鐘）驗證並自動重建容器：

- 路徑必須位於 `/share/<共享或磁碟區>/` 之下，含特殊字元或不存在的音樂路徑會被拒絕並記錄到系統事件；
- 表單經由 UI 容器內的極簡 CGI 落地成待處理檔，**解析與驗證全在主機端腳本**完成，
  UI 容器本身無法接觸 docker 或主機設定；
- 變更資料庫路徑不會自動搬移舊資料（搬移步驟見上節）。
...
| Roon 新增 SMB 網路共享出現 `UnexpectedError` | 這是預期行為：Roon 跑在容器內，無法（也不需要）掛載 SMB。請改用 Roon「設定 → 儲存位置 → **新增資料夾**」選擇 `/Music`；要換音樂資料夾請用狀態頁的「資料夾設定」。 |
| 系統記錄出現 Music path does not exist | 預設音樂路徑猜錯（1.2.0 起會自動從 smb.conf 偵測 Multimedia/Music/Public 共享）。到狀態頁「資料夾設定」把音樂庫指向實際的共享資料夾即可。 |
